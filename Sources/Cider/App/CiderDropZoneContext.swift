...
final class CiderDropZoneContext: ObservableObject {
    ...
    func setTargeted(_ isTargeted: Bool) {
        status = isTargeted ? .targeted : .idle
    }
    ...
    func saveDroppedImageData(_ data: Data, preferredFileExtension: String? = nil) {
        status = .processing("Saving dropped image...")
        let bookmark = VaultBookmarkService.shared.addImageBookmark(title: "Dropped Image")
