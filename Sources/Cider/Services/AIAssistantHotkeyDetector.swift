...
/// Detects Option+A and posts `.toggleAIAssistantPanel`.
...
final class AIAssistantHotkeyDetector: @unchecked Sendable {

    private let logger = Logger(subsystem: "com.cider.app", category: "AIAssistantHotkeyDetector")
    ...
    func start() {
        ...
        eventTap = CGEvent.tapCreate(
            ...
            callback: { proxy, type, event, refcon in
                ...
                let detector = Unmanaged<AIAssistantHotkeyDetector>.fromOpaque(refcon).takeUnretainedValue()
        ...
        guard let tap = eventTap else {
            if registerHotKeyFallback() {
                logger.info("AI assistant hotkey detector started (Carbon hotkey fallback)")
            } else {
        ...
        logger.info("AI assistant hotkey detector started")
    }
    ...
    private static let signature: OSType = 0x41494153 // "AIAS"
    ...
    private func registerHotKeyFallback() -> Bool {
        ...
        let status = InstallEventHandler(
            ...
            { _, eventRef, userData in
                ...
                let d = Unmanaged<AIAssistantHotkeyDetector>.fromOpaque(userData).takeUnretainedValue()
