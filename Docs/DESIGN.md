...
- Library should open to the complete visual collection and use subtle, low-weight multi-select pills for fast show/hide filtering. Keep item-type filters, workflow state, and Space/entity lenses conceptually distinct so the control row stays calm. Park Folders and Tags from default Library UI while preserving all data/backend capability; reintroduce only the smallest proven surface if real organization friction appears.
...
- Cider Chat should default to one calm human-facing room transcript with one clearly identified head agent. Separate three controls that must not be conflated: who participates, who receives an unaddressed message, and whose output is visible. Agent work may live in linked per-run detail or an agent-filtered work view, while the main transcript defaults to head-agent summaries and final outcomes. Per-agent visibility may be inline, summary-only, or hidden, but hidden work must still surface failures, approval requests, and completion state. Switching a room's head agent preserves canonical history and must not expose provider credentials or runtime administration.
...
- Compact overlays should be discoverable without permanently stealing space. Administrative views such as Browse All Boards belong under a secondary More menu rather than primary navigation. Contextual tools such as the floating Test Checklist should appear only from relevant Testing cards.
