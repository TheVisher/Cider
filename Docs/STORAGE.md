...
- Native room speech input uses a Cider-owned provider-neutral transcription boundary and the existing per-canonical-room draft store. Apple Speech is the initial explicitly started on-device implementation; permission is requested only from the microphone control, and denied, restricted, offline, unavailable, cancelled, or failed sessions restore the exact pre-session typed draft without a turn, message, transport call, or provider fallback. Bounded partial/final text is deterministically appended to only the originating room's editable draft, room changes cancel and invalidate late callbacks, and transcription completion never submits. Only the existing explicit Send path can create canonical history. Process-local speech provenance records provider, originating room, bounded transcript, and finality; raw audio and speech-specific provenance are not persisted by default.
