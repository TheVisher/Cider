...
- Native room speech input composes Cider's shared provider-neutral transcription capability and the existing per-canonical-room draft store. Apple Speech is the current explicitly started on-device adapter for both live microphone and caller-supplied stored-audio inputs, with no silent network fallback. Permission is requested only from an explicit surface action; denied, restricted, offline, unavailable, cancelled, or failed live sessions restore the exact pre-session typed draft without a turn, message, transport call, or provider fallback. Bounded partial/final text is deterministically appended to only the originating room's editable draft, room changes cancel and invalidate late callbacks, and transcription completion never submits. Only the existing explicit Send path can create canonical history. Process-local Chat speech provenance records provider, source, locale, timing, bounded transcript, and finality; Chat does not retain raw microphone audio or persist speech-specific provenance by default. The shared stored-audio contract carries a stable caller-owned source identity and `preserveOriginal` retention requirement while treating its file URL as a read-only input handle; future Journal and transport consumers must preserve the immutable original through their own canonical capture path and may edit only the derived transcript.
