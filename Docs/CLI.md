...
- `item batch-apply --stdin --approve <token> --execute --json`: applies an approved batch through existing typed item mutation APIs. The MVP apply path supports `move` and `unfile`; `route` and `link` operations can be planned but are skipped until they have equivalent typed batch apply tests. JSON returns per-operation before/after payloads, mutation audit IDs where available, partial failures, and rollback/repair guidance.
