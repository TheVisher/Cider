...
For general backup safety changes, use fresh disposable physical roots and fingerprint the source DB/WAL/SHM set, every prior retained package, and the package DB/manifest before and after verification and each injected failure boundary. Cover same-second same-reason publication, a real committed WAL plus an in-flight writer transaction, current and older schemas, immutable physical reopen, malformed and unhealthy output, staging/capture failure after the first filesystem side effect, destination collision, publication failure, and retained verification failure. Require distinct package identities, exact retained-byte stability, truthful typed receipts, and no pruning until the new package has been atomically published and re-verified. At each cleanup or pruning boundary, replace the pathname occupant after the final observation and prove that Cider either removes only the continuously pinned identity or fails closed with the exact retained URL; never require a check-then-pathname-delete cleanup that can remove a later occupant.

For restore changes, build a source × destination × operation matrix before implementation. Cover owned v2, eligible raw/v1 recovery, malformed, orphan, moved/replaced, and emitted rollback sources against DB-only, DB/WAL/SHM, absent, unexpected sidecar, failed reopen/integrity, and interrupted destinations. A rejection at list or verification must remain rejected by URL and qualified materialization, CLI selection, live restore, and receipt validation. Put FIFO, socket, directory, and symlink occupants at both `manifest.json` and every supported raw `.db` acquisition and require bounded-process prompt rejection with exact occupant/source/live-DB preservation. On disposable roots, independently inject before/after intent publication, exclusive staging creation before durable identity, existing/absent publication, DB and sidecar transition, the first directory sync, private immutable SQLite validation, commit, normal post-commit open, compensation, terminal retention, and process interruption at real production boundaries. Forge every v2 state with arbitrary or duplicate names, duplicate and cross-role identities, DB-as-sidecar reuse, and source/member overlap. At rename and clone, mutate a pinned inode in place without changing size immediately beside the syscall and require fail-closed preservation. Count every unlink attempt for canonical `cleanup-retained` names and require zero through completion, compensation, interruption, and two restarts; hold a writable descriptor through the final retention proof, change same-size bytes, and prove the retained name still reaches those exact bytes. Exercise hard-cap refusal before mutation, terminal-set nonselection/nonmaterialization, repeated-restart no-growth, and both retained-destination collision and source-name reoccupation. Use a structurally valid committed WAL/SHM installed before the actual open boundary rather than marker bytes. Fingerprint directory and file identities, content, xattrs, exact transaction bytes and generated graph, ownership ledger, packages, terminal evidence, and complete DB/WAL/SHM before, after, and after two restarts. Exercise equivalent real CLI JSON and text cases where the rollback selector moves, is replaced, or loses ownership eligibility immediately before serialization; both formats must omit stale guidance or fail. Only a currently listed and immutably verified package may appear as `recoverySelector`, and no failed path may report `ok:true`, `changed:true`, or a usable success receipt.
