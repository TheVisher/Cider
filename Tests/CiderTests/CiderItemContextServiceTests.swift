...
struct CiderItemContextServiceTests {
    ...
    func capturedItemContextExposesCaptureSourceContext() throws {
        ...
        #expect(provenance[0]["projection"] as? String == "cli_default")
        #expect(provenance[0]["containsPrivateData"] as? Bool == false)
        #expect(provenance[0]["surfaceCategory"] as? String == "other")
        #expect(provenance[0]["channelCategory"] as? String == "cli")
        #expect(provenance[0]["hasMessageID"] as? Bool == true)
        #expect(provenance[0]["messageID"] == nil)

        let trustedDict = CiderCLI.itemContextBundleToDict(bundle, provenanceProjection: .trustedLocal)
        let trustedProvenance = try #require(trustedDict["captureProvenance"] as? [[String: Any]])
        #expect(trustedProvenance[0]["projection"] as? String == "trusted_local")
        #expect(trustedProvenance[0]["surface"] as? String == "codex_dogfood")
        #expect(trustedProvenance[0]["channel"] as? String == "cli")
        #expect(trustedProvenance[0]["messageID"] as? String == "dogfood-2026-05-19")
