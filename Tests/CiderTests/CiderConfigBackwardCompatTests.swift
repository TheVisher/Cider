...
struct CiderConfigBackwardCompatTests {
    ...
    func emptyJSON() throws {
        ...
        #expect(config.activationSpeed == 0.3)
        ...
        #expect(config.confirmCopiedURLBeforeSave == false)
        #expect(config.autoCaptureCopiedImages == false)
        ...
        #expect(config.bookmarksCardSize == .comfortable)
        #expect(config.notesDefaultViewMode == .list)
        #expect(config.detailViewMode == .slideOut)
        #expect(config.showContinueSection == true)
        #expect(config.continueSectionCollapsed == false)
        #expect(config.subFoldersCollapsed == false)
        #expect(config.folderOverviewCollapsedByParentID.isEmpty)
        #expect(config.homeDisplayMode == .list)
        #expect(config.tagsCollapsed == false)
        #expect(config.enableLinkedSources == false)
        #expect(config.trashRetentionDays == 30)
        #expect(config.captureToastPosition == .topCenterScreen)
        #expect(config.undoToastPosition == .bottomRightPanel)
        #expect(config.homeSort == .createdDescending)
        #expect(config.homeEntityFilter == LibraryEntityType.activeCases)
        ...
        #expect(config.showDragModeHints == true)
        ...
        #expect(config.openOnMouseScreen == false)
        #expect(config.sessionRestoreBrowserBundleID == nil)
        #expect(config.tableColumnConfig == .default)
    }
