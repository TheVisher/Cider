...
struct KanbanBoardFileLockingTests {
    ...
    @Test("board create help flag after name does not create board")
    func boardCreateHelpFlagAfterNameDoesNotCreateBoard() throws {
        let cli = try #require(Self.ciderCLIURL())
        let vault = try Self.makeTemporaryVault()
        defer { try? FileManager.default.removeItem(at: vault) }

        let result = try Self.runCLIResult(cli, vault: vault, args: ["board", "create", "No Mutation", "--help"])
        let output = String(data: result.output, encoding: .utf8) ?? ""
